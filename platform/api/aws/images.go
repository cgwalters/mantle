...
func (a *API) CreateHVMImage(snapshotID string, name string, sizeGB uint32, description string) (string, error) {
	params := registerImageParams(snapshotID, name, sizeGB, description, "xvd", EC2ImageTypeHVM)
	params.EnaSupport = aws.Bool(true)
	params.SriovNetSupport = aws.String("simple")
	return a.createImage(params)
}

func (a *API) CreatePVImage(snapshotID string, name string, sizeGB uint32, description string) (string, error) {
	if !RegionSupportsPV(a.opts.Region) {
		return "", NoRegionPVSupport
	}
	params := registerImageParams(snapshotID, name, sizeGB, description, "sd", EC2ImageTypePV)
...
func registerImageParams(snapshotID string, name string, sizeGB uint32, description string, diskBaseName string, imageType EC2ImageType) *ec2.RegisterImageInput {
		...
		BlockDeviceMappings: []*ec2.BlockDeviceMapping{
			&ec2.BlockDeviceMapping{
				...
				Ebs: &ec2.EbsBlockDevice{
					...
					VolumeSize:          aws.Int64(int64(sizeGB)),
